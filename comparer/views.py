...
def _fetch_and_process_city_weather_data(city_name, year):
    ...
    if not coordinates:
        city_processing_result['error'] = f"Could not find coordinates for '{city_name}'. Please check the name."
        return city_processing_result
    ...
    if not annual_weather_data or not annual_weather_data.get('monthly_data'):
        city_processing_result['error'] = f"Could not retrieve annual weather data for {city_name} for {year}."
        return city_processing_result
...
def index_view(request):
    ...
    if request.method == 'POST':
        ...
        city1_name, city2_name, selected_year, form_error = _parse_form_input(request.POST)
        ...
        city1_processed_data = _fetch_and_process_city_weather_data(city1_name, selected_year)
        city2_processed_data = _fetch_and_process_city_weather_data(city2_name, selected_year)
        
        context['city_details_for_display'] = [city1_processed_data, city2_processed_data]
        ...
        if city1_processed_data.get('weather_data_raw'):
            chart_data_for_python_list.append({
                ...
                'temperatures': [m['avg_temp'] for m in city1_processed_data['weather_data_raw']],
                'precipitations': [m['total_precip'] for m in city1_processed_data['weather_data_raw']],
        ...
        if city2_processed_data.get('weather_data_raw'):
             chart_data_for_python_list.append({
                ...
                'temperatures': [m['avg_temp'] for m in city2_processed_data['weather_data_raw']],
                'precipitations': [m['total_precip'] for m in city2_processed_data['weather_data_raw']],
        ...
        if not chart_data_for_python_list and not form_error: # Check the Python list
            all_errors = [d['error'] for d in context['city_details_for_display'] if d['error']]
            if len(all_errors) == 2 :
                 context['error_message'] = "Could not generate chart data. Weather data might be unavailable or city names incorrect for both entries."
